...
class TaskExecutor:
    ...
    def _evaluate_changed_when_result(self, templar, result, vars_copy):
    ...
    def _evaluate_failed_when_result(self, templar, result, vars_copy):
    ...
    def _execute(self, variables=None, from_loop=False):
        ...
        for attempt in range(1, retries + 1):
            ...
            # Projection not needed at this point
            # if not from_loop:
            ...
            if 'skipped' not in result:
                ...
                try:
                    self._evaluate_changed_when_result(templar, result, vars_copy)
                    condname = 'failed'
                    self._evaluate_failed_when_result(templar, result, vars_copy)
                except AnsibleError as e:
                    result['failed'] = True
